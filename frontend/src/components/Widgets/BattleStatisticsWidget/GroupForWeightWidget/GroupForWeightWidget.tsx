...
import './groupForWeightWidget.scss';

const currentConfig = BattleConfig[BattleType.GROUP_BY_WEIGHT];

export function GroupForWeightWidget({ entity }: { entity?: IBattleInfo }) {
    const t = useTranslations('GroupForWeightWidget');
    const tBattleConfig = useTranslations('BattleConfig.' + BattleType.GROUP_BY_WEIGHT);
    ...
    const currentStatus = useMemo(() => {
        return entity?.status || BattleStatusEnum.LOST; //
    }, [entity]);
    ...
    return (
        <div className={`battle-widget group-for-weight-widget battle-status_${currentStatus}`}>
            <div className="group-for-weight-widget__top">
                <BattleImage battle_type={currentConfig!.type} />
                {currentStatus === BattleStatusEnum.NEW && (
                    <div className="group-for-weight-widget__info">
                        <h3>
                            {tBattleConfig('Title')}{' '}
                ...
                {currentStatus === BattleStatusEnum.WAITING && (
                    <div className="group-for-time-widget__info">
            ...
            <div className="group-for-weight-widget__controls">
                {currentStatus === BattleStatusEnum.NEW && (
                    <Button
                        variant={'orange'}
