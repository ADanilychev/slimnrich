...
import { IWithGroupQuery } from '@/lib/types/services/battles.type';

import { useRouter } from '@/i18n/routing';
import { BattleService } from '@/services/battle.service';

export function WithYourGroup() {
    const t = useTranslations('CreateBattle.WithYourGroup');
    ...
    const { handleSubmit, watch, control } = useForm<IWithGroupQuery>({
        mode: 'onChange',
        defaultValues: {
    ...
    const { mutateAsync, isPending: isPendingCreateBattle } = useMutation({
        mutationKey: ['create-with-your-group-battle'],
        mutationFn: async (payload: IWithGroupQuery) => await BattleService.createWithYourGroup(payload),
    });

    const onSubmit: SubmitHandler<IWithGroupQuery> = async (data) => {
        mutateAsync(data, {
            onSuccess: () => {
                queryClient.invalidateQueries({ queryKey: ['get-user-basic-data'] });
                queryClient.invalidateQueries({ queryKey: ['fetch-user-battles'] });
                startTransition(() => {
                    router.push(BATTLE_PAGE.PLAN(BattleType.WITH_YOUR_GROUP));
                });
            },
            onError: async (error) => {
        ...
        <div className="custom-battle one-on-one-with-friend">
            ...
            <form className="battle-form" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="custom-battle__footer">
                    <Button type="submit" variant={'active'} isLoading={isPendingCreateBattle}>
                        {t('Pay')}
                    </Button>
