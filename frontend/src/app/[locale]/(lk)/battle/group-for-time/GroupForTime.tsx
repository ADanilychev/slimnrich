...
export function GroupForTime() {
    const t = useTranslations('CreateBattle.GroupForTime');
    ...
    const { mutateAsync, isPending: isPendingCreateBattle } = useMutation({
        mutationKey: ['create-group-time-battle'],
        mutationFn: async (payload: IVSQuery) => BattleService.createGroupForTime(payload),
    });
    ...
    const onSubmit: SubmitHandler<IVSQuery> = async (data) => {
        mutateAsync(data, {
            onSuccess: () => {
                ...
                startTransition(() => {
                    router.push(BATTLE_PAGE.PLAN(BattleType.GROUP_FOR_TIME));
                });
            },
            onError: async (error) => {
    ...
    return (
        <div className="custom-battle group-for-time-battle">
            ...
            <form className="battle-form" onSubmit={handleSubmit(onSubmit)}>
                <div className="custom-battle__content">
                    ...
                    <InputWithTitle
                        title={t('Duration')}
                        placeholder={`4 ${t('Weeks')}`}
                        defaultValue={`4 ${t('Weeks')}`}
                        block
                    />
                </div>
